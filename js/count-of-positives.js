...
function countPositivesSumNegatives(input){
  ...
  input.forEach(item, index => {
      let arrPositives = input.slice(0, index)
      let arrNegatives = input.slice(index + 1).reduce((acc, c) => acc + c, 0)
      if(arrPositives == true){
        return newArray = newArray.push(arrPositives.length)
      }else{
          return newArray = newArray.push(arrNegatives)
      }
  });
  return newArray
}
