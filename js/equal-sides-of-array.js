...
// of the array, the sum of left side of the index ({1}) and the sum of the right 
...
function findEvenIndex(arr){
  ...
  arr.forEach(item, index =>{
      let leftSide = arr.slice(0, index).reduce((acc, c) => acc + c, 0)
      let rightSide = arr.slice(index + 1).reduce((acc, c) => acc + c, 0) //slice grabs the index after the left side and adds 1 to it to grab the index of the right side
